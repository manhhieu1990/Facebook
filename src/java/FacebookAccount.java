...
public class FacebookAccount {
    ...
    public FacebookAccount(String uid, String fname) {
        ...
        try {
            ...
            rs = stat.executeQuery("Select * from facebook_status where user_ID IN (Select follow_user_ID from facebook_follow where user_ID = '" + id + "') or user_ID = '" + id + "' order by status_time DESC");
            while (rs.next()) {
                String status_ID = rs.getString(1);
                String comment_ID = rs.getString(3);
                String comment_userID = rs.getString(2);
                String share_this_message = rs.getString(4);
                String comment_time = rs.getString(5);
                Status s = new Status(status_ID, comment_ID, comment_userID, share_this_message, comment_time);
                ...
                rs1 = stat1.executeQuery("Select * from facebook_reply where reply_facebook_ID = '" + status_ID + "' and reply_user_ID = '" + id + "'");
                if (rs1.next()) {
                ...
                while (rs1.next()) {
                    Reply r = new Reply(status_ID, rs1.getString(3), rs1.getString(4), rs1.getString(5));
                    statusReplies.add(r);
                }
    ...
    public String status(String pageToDirect) {
        ...
        try {
            conn = DriverManager.getConnection(DB_URL, "root", "");
            stat = conn.createStatement();
            int j = stat.executeUpdate("Insert into facebook_status (user_ID, status_message) values ('" + id + "'" + status_message + "')");
            status++;
            ArrayList<String> hashTags = new ArrayList<String>();
            String[] sub_status_message = status_message.split("#");
            for (int i = 1; i < sub_status_message.length; i++) {
                String sub_status_message_each = sub_status_message[i];
            ...
            while (rs.next()) {
                ...
                rs1 = stat1.executeQuery("Select * from status_reply where reply_status_id = '" + status_ID + "' order by reply_timestamp asc");
                while (rs1.next()) {
                    Reply r = new Reply(status_ID, rs.getString(3), rs1.getString(4), rs1.getString(5));
                    statusReplies.add(r);
                }
                s.setReplies(statusReplies);
                userStatus.add(s);
            ...
            trend.clear();
            rs = stat.executeQuery("Select * from status_trending order by trending_count desc");
            int counter = 1;
            while (rs.next() && counter < 6) {
                trend.add(new TrendingTopics(rs.getString(1), rs.getInt(2)));
                counter++;
        ...
        try {
            ...
            if (rs.next()) {
                ...
                while (rs.next()) {
                    ...
                    ArrayList<Reply> statusReplies = new ArrayList<Reply>();
                    rs1 = stat1.executeQuery("Select * from status_replu where like_status_ID = '" + status_ID + "'");
                    while (rs1.next()) {
                        Reply r = new Reply(status_ID, rs1.getString(3), rs1.getString(4), rs1.getString(5));
                        statusReplies.add(r);
                    }
    ...
    public String follow() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Status s = new Status(status_ID, share_ID, statusUser_ID, status_message, status_time);
                stat1 = conn.createStatement();
                rs1 = stat1.executeQuery("Select * from facebook_like where like_status_ID = '" + status_ID + "' and like_user_ID = '" + id + "'");
                if (rs1.next()) {
                    s.setDisplayLikeButton(false);
                } else {
                    s.setDisplayLikeButton(true);
                }
                rs1 = stat1.executeQuery("Select * from facebook_reply where reply_status_ID = '" + status_ID + "' and reply_user_ID = '" + id + "'");
                if (rs1.next()) {
                    s.setDisplayReplies(true);
                } else {
                    s.setDisplayReplies(false);
                }
                rs1 = stat1.executeQuery("Select count(1) from facebook_like where like_status_ID = '" + status_ID + "'");
                if (rs1.next()) {
                    s.setLike_count(rs1.getInt(1));
                } else {
                    s.setLike_count(0);
                ...
                ArrayList<Reply> statusReplies = new ArrayList<Reply>();
                rs1 = stat1.executeQuery("Select * from facebook_reply where reply_status_id = '" + status_ID + "' order by reply_timestamp asc");
                while (rs1.next()) {
                    Reply r = new Reply(status_ID, rs1.getString(3), rs1.getString(4), rs1.getString(5));
                    statusReplies.add(r);
                }
    ...
    public String unFollow() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                rs1 = stat1.executeQuery("Select * from status_reply where reply_status_ID = '" + status_ID + "' abd reply_user_ID = '" + id + "'");
                if (rs1.next()) {
                    s.setLike_count(rs1.getInt(1));
                } else {
                    s.setLike_count(0);
...
//end method share
    ...
    public String likeStatus(Status s, String pageToRedirect) {
        ...
        try {
            conn = DriverManager.getConnection(DB_URL, "root", "");
            stat = conn.createStatement();
            int i = stat.executeUpdate("Insert into facebook_like (like_status_ID like_user_ID) values ('" + s.getStatus_ID() + "','" + id + "')");
            s.setDisplayLikeButton(false);
    ...
    public String unlikeStatus(Status s, String pageToRedirect) {
        ...
        try {
            conn = DriverManager.getConnection(DB_URL, "roor", "");
            stat = conn.createStatement();
            int i = stat.executeUpdate("Delete from facebook_like where like_status_ID = '" + s.getStatus_ID() + "' and like_user_ID = '" + id + "'");
            s.setDisplayLikeButton(true);
            int statusLikeCount = s.getLike_count();
            s.setLike_count(statusLikeCount - 1);
            return (pageToRedirect);
    ...
    public String replyStatus(Status s1, String pageToRedirect) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ArrayList<Reply>statusReplies = new ArrayList<Reply>();
                rs1 = stat1.executeQuery("Select * from status_reply where reply_status_id = '" + status_ID + "' order by reply_timestampe asc");
                while(rs1.next()){
                    Reply r = new Reply(status_ID,rs1.getString(3),rs1.getString(4),rs1.getString(5));
                    statusReplies.add(r);
                }
